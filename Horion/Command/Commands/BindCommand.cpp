...
bool BindCommand::execute(std::vector<std::string>* args) {
	...
	if (!modOpt.has_value()) {
		clientMessageF("Could not find module with name: %s%s", GRAY, moduleName.c_str());
		return true;
	}
	...
	if (args->size() >= 3) {
		...
		if (key.size() > 1) {
			...
			if (key == "none") {
				mod->setKeybind(0x0);
				clientMessageF("%sSuccessfully unbound %s%s%s!", GRAY, mod->getModuleName(), RESET);
				return true;
			}
			...
			for (int i = 0; i < 190; i++) {
				...
				if (strcmp(needle, haystackLowercase) == 0) {
					mod->setKeybind(i);
					clientMessageF("The keybind of %s%s%s is now '%s%s%s'", GRAY, mod->getModuleName(), RESET, GRAY, haystack, RESET);
					delete[] haystackLowercase;
					return true;
				}
				delete[] haystackLowercase;
			}
			clientMessageF("Invalid key!");
			return true;
		}
		...
		if (keyCode >= 0x30 && keyCode <= 0x5A) {
			auto modOpt = moduleMgr->getModuleByName(moduleName);
			if (!modOpt.has_value()) {
				clientMessageF("Could not find module with name: %s'%s%s,", GRAY, moduleName.c_str(), RESET);
			} else {
				modOpt.value()->setKeybind(keyCode);
				clientMessageF("The Keybind of %s%s%s is now '%s%c%s'", GRAY, mod->getModuleName(), RESET, GRAY, keyCode, RESET);
			}
		} else {
			clientMessageF("Invalid Key! Outside of ascii range: %s%X", GRAY, keyCode);
		}
		return true;
	} else {
		clientMessageF("%s%s%s is currently bound to %s'%s%s'", GRAY, mod->getModuleName(), RESET, GRAY, Utils::getKeybindName(mod->getKeybind()), GRAY);
		return true;
	}
}