...
void ConfigManager::loadConfig(std::string name, bool create) {
	...
	sprintf_s(fullPath, allocSize, "%S\\Horion\\%s.h", roamingFolder.c_str(), name.c_str());
	
	const bool configExists = std::filesystem::exists(fullPath);
	if (!configExists) {
		static auto tabGuiModule = moduleMgr->getModule<TabGuiMod>("TabGui");
		static auto arraylistModule = moduleMgr->getModule<Arraylist>("Arraylist");
		tabGuiModule->setEnabled(true);  //Enable if first time use
		arraylistModule->setEnabled(true);
	}
	if (configExists || create) {
		...
		if (Game.getLocalPlayer()) {
			...
			Game.getGuiData()->displayClientMessageF("%s[%sHorion%s] - %sSuccessfully %s config %s%s%s!", GRAY, AQUA, GRAY, RESET, !configExists ? "created" : "loaded", GRAY, name.c_str(), RESET);
			Game.getGuiData()->displayClientMessageF("%s[%sHorion%s] - %sDefault keybind is %sinsert. %sUse %s.bind clickgui (key) %sto rebind it!", GRAY, AQUA, GRAY, RESET, GRAY, RESET, GRAY, RESET);
			if (!helpedUser && name != "default") {
				helpedUser = true;
				Game.getGuiData()->displayClientMessageF("%s[%sHorion%s] - %sEnter %s%cconfig load default%s to load your old config!", GRAY, AQUA, GRAY, RESET, GRAY, cmdMgr->prefix, RESET);
			}
		}
	} else {
		if (Game.getLocalPlayer()) 
			Game.getGuiData()->displayClientMessageF("%s[%sHorion%s] - %sCould not load config %s%s%s!", GRAY, AQUA, GRAY, RED, GRAY, name.c_str(), RED);
	}
...
void ConfigManager::saveConfig() {
	...
	char* fullPath = new char[allocSize];
	sprintf_s(fullPath, allocSize, "%S\\Horion\\%s.h", roamingFolder.c_str(), currentConfig.c_str());
