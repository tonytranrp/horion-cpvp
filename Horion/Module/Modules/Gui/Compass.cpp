...
Compass::Compass() : IModule(0, Category::GUI, "Compass") {
...
void Compass::onPreRender(MinecraftUIRenderContext* renderCtx) {
	...
	if (!player || !GameData::canUseMoveKeys()) return;

	static auto wpMod = moduleMgr->getModule<Waypoints>("Waypoints");
	...
	const int deg = (int)(player->viewAnglesPtr->viewAngles.y + 180);
	...
	for (int off = -range; off <= range; off++) {
		...
		for (auto it = result.first; it != result.second; it++) {
			...
			const float tWidth = DrawUtils::getTextWidth(&pName, tSize, DesiredUi::HUD);
			pos.x -= tWidth / 2;
			...
			DrawUtils::drawText(pos, &pName, MC_Color(255, 255, 255), tSize, majorOpacity, DesiredUi::HUD);
		}

		if (off != 0 && minorOpacity > 0) {
			if ((oDeg % 15) ) {
				if ((oDeg % 5) == 0) {
					DrawUtils::fillRect(Vec4(xOff - 0.25f, 16, xOff + 0.25f, 19), MC_Color(255, 255, 255), minorOpacity);
				}
				continue;
			}

			// Bigger line with degree displayed
			DrawUtils::fillRect(Vec4(xOff - 0.5f, 15, xOff + 0.5f, 20), MC_Color(255, 255, 255), minorOpacity);
			drawCenteredText(Vec2(xOff, 20.f), std::to_string(oDeg), 0.75f, minorOpacity);
		}
	}

	// Center line
	DrawUtils::fillRect(Vec4(sCenter - 0.5f, 15, sCenter + 0.5f, 25), MC_Color(255, 255, 255), opacity);
...
void Compass::drawCenteredText(Vec2 pos, std::string text, float size, float textOpacity) {
	pos.x -= DrawUtils::getTextWidth(&text, size, DesiredUi::HUD) / 2;
	DrawUtils::drawText(pos, &text, MC_Color(255, 255, 255), size, textOpacity, DesiredUi::HUD);
}