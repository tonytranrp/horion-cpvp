...
Radar::Radar() : IModule(0, Category::GUI, "Radar") {
...
void renderEntity(Entity* currentEntity) {
	...
	if (!player->canAttack(currentEntity))
		return;
	...
	if (relPos.x > 0 && relPos.x < size && relPos.y > topPad - cent && relPos.y < topPad + cent) {
		DrawUtils::fillRect(Vec4(relPos.x - pxSize, relPos.y - pxSize, relPos.x + pxSize, relPos.y + pxSize), MC_Color(rcolors[0], rcolors[1], rcolors[2]), pxOpacity);
	}
}
...
void Radar::onPreRender(MinecraftUIRenderContext* renderCtx) {
	...
	if (!player || !Game.getLevelRenderer()) return;
	...
	DrawUtils::fillRect(Vec4(0, topPad - cent, (float)size, topPad + cent), MC_Color(0, 0, 0), bgOpacity);

	auto pPos = Game.getLevelRenderer()->getOrigin();
	playerPos = pPos;

	angle = (180.0f - player->viewAnglesPtr->viewAngles.y) + 180.0f;
