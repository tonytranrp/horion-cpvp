...
void InventoryCleaner::onTick(GameMode* gm) {
	...
	if (!dropSlots.empty()) {
		for (int i : dropSlots) {
				Game.getInventory()->dropSlot(i, false, true, false);
		}
	}
...
std::vector<int> InventoryCleaner::findStackableItems() {
	...
	for (int i = 0; i < 36; i++) {
		ItemStack* itemStack = Game.getInventory()->getItemStack(i);
		if (itemStack->item) {
			if ((*itemStack->item)->getMaxStackSize(0) > itemStack->count) {
				for (int i2 = 0; i2 < 36; i2++) {
					if (i2 == i) continue;
					ItemStack* itemStack2 = Game.getInventory()->getItemStack(i2);
...
std::vector<int> InventoryCleaner::findUselessItems() {
	...
	if (items.size() > 0) {
		...
		for (int i = 0; i < 36; i++) {
			if (std::find(uselessItems.begin(), uselessItems.end(), i) != uselessItems.end())
				continue;
			ItemStack* itemStack = Game.getInventory()->getItemStack(i);
			if (itemStack->item && itemStack->getAttackingDamageWithEnchants() > 1) {
	...
	// Filter armor
	{
		...
		for (int i = 0; i < 36; i++) {
			...
			ItemStack* itemStack = Game.getInventory()->getItemStack(i);
			if (itemStack->item && (*itemStack->item)->isArmor()) {
...
bool InventoryCleaner::stackIsUseful(ItemStack* itemStack) {
	...
	if (keepTools && (*itemStack->item)->itemId == 568) return true;  // Totem
	if (keepTools && (*itemStack->item)->itemId == 345) return true;  // Compass
	if (keepTools && (*itemStack->item)->itemId == 450) return true;  // Totem
	if (keepTools && (*itemStack->item)->itemId == 264) return true;  // Diamonds!
	return false;
}
...
bool InventoryCleaner::isLastItem(Item* item) {
	...
	for (int i = 0; i < 36; i++) {
		...
		if (stack->item) items.push_back((*stack->item));
	}
	int count = 0;
	for (Item* a : items) {
		if (a == item) count++;
