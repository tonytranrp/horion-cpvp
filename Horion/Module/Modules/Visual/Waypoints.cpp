...
void Waypoints::onPreRender(MinecraftUIRenderContext* renderCtx) {
	...
	if (!localPlayer || !GameData::canUseMoveKeys())
		return;
	int currentDimension = localPlayer->getDimensionId();
	...
	for (auto it = waypoints->begin(); it != waypoints->end(); it++) {
		...
		if (fadeOutAtDistance && dist > 15) {
			Vec2 angle = localPlayer->stateVector->pos.CalcAngle(pos);
			float diff = angle.sub(localPlayer->viewAnglesPtr->viewAngles).normAngles().magnitude();
			if (dist > 30) {
		...
		float textWidth = DrawUtils::getTextWidth(&txt, size, DesiredUi::HUD) + 0.5f;
		float textHeight = DrawUtils::getFont(DesiredUi::HUD)->getLineHeight() * size;
		...
		if (textPos.x != -1) {
			...
			if (this->showCoordinates) {
				...
				coordText = out.str();
				textWidth = std::max(textWidth, DrawUtils::getTextWidth(&coordText, size * 0.75f, DesiredUi::HUD) + 0.5f);
			}
			...
			DrawUtils::fillRect(rectPos, color, alpha * 0.5f);
			DrawUtils::drawText(textPos, &txt, MC_Color(255, 255, 255), size, alpha, DesiredUi::HUD);
			if (this->showCoordinates) {
				textPos.y += textHeight + 1.f * size;

				DrawUtils::drawText(textPos, &coordText, MC_Color(255, 255, 255), size * 0.75f, alpha, DesiredUi::HUD);
			}
		}
		DrawUtils::Flush();
	}
