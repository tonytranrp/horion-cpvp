...
void JoeMovementController::step(LocalPlayer *player, MoveInputHandler *movementHandler) {
	...
	if (!curSeg.isInValidPosition(playerNode) && curSeg.getSegmentType() != PARKOUR_JUMP) {
	...
	case DROP: {
		...
		if (player->isOnGround() || inWater) {
			dComp = 1;
			if(fabsf(pPos.y - end.y) < (inWater ? 0.2f : 0.1f) && pPos.sub(end).magnitudexz() < 0.5f && player->stateVector->velocity.y > -0.1f){// Check for end condition
				this->stateInfo.nextSegment();
				break;
			}else if(inWater){
				if(pPos.y < end.y || player->stateVector->velocity.y < 0.12f)
					movementHandler->isJumping = 1;
			}
		}else{
			dComp = 3;
		...
		/* if (start.sub(end).magnitudexz() < 1.1f) {
			...
			auto tangent = end.sub(start);
			tangent.y = 0;
			walkTarget = start.add(tangent.mul(0.5f + player->aabbShape->width * 0.5f + 0.1f));
			walkTarget.y = end.y;
		}*/
	...
	case PARKOUR_JUMP: {
		...
		if (player->isOnGround()) {
			...
			if(start.sub(end).magnitudexz() <= 1.1f)
				maxJumpDist = 0.7f;
			if(posToJumpTarg < maxJumpDist && posToJumpTarg > 0 && player->stateVector->velocity.dot(tangent) > 0.07f){
				// jump
	...
	case WATER_WALK: {
		{
			...
			auto followPathMod = moduleMgr->getModule<FollowPathModule>("FollowPath");
			auto jesusMod = moduleMgr->getModule<Jesus>("Jesus");
			...
			if(isVertical){
				...
				if (followPathMod->assumeWaterWalk && player->isInWater()) {
					player->setSprinting(curSeg.isAllowingSprint());
					if (!jesusMod->isEnabled()) jesusMod->setEnabled(true);
					jesusMod->solid = true;
					player->stateVector->velocity.y = 0.30f;
				} else if (player->isInWater()) movementHandler->isJumping = 1;

				if (!player->isOverWater()) jesusMod->setEnabled(false);
			...
			auto block = Game.getRegion()->getBlock(playerNode);
			if(!block->toLegacy()->material->isLiquid){
				auto mod = playerNode.add(0, -1, 0);
				block = Game.getRegion()->getBlock(mod);

				if(block->toLegacy()->material->isLiquid)
					block->toLegacy()->liquidGetFlow(&flow, Game.getRegion(), &mod);
			}else{
				block->toLegacy()->liquidGetFlow(&flow, Game.getRegion(), &playerNode);
			}
	...
	case WALK: {
		...
		pPosD = pPosD.add(player->stateVector->velocity.mul(dComp, 0, dComp));  // d

		if (player->isOnGround() && end.y < start.y && fabsf(start.y - pPosD.y) < 0.1f && player->remainingTicksUsingItem > 0 && end.sub(start).magnitudexz() > 1.5f) {
			// drop with a gap
		...
		if(pPos.dist(end) < 0.2f){
			if(hasNextSeg || player->stateVector->velocity.magnitudexz() < 0.02f /*slow down for last segment*/){
				this->stateInfo.nextSegment();
				break;
			}
		}
