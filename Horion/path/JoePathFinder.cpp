...
__forceinline bool isDangerous(const Vec3i& pos, BlockSource* reg, bool allowWater){
	...
	// contact damage based
	{
		...
		if (obs1->tileName.size() < 6)
			return false;

		for (int i = 0; i < dangerousTiles.size(); i++){
			const char* tile = dangerousTiles[i];
			if (strcmp(obs1->tileName.c_str() + 5 /*cutoff tile. prefix*/, tile) )
				continue;
...
__forceinline bool canStandOn(const Vec3i& pos, BlockSource* reg, bool inWater = false){
	...
	AABB aabb;
	if(!standOn->getCollisionShape(&aabb, block, reg, &pos))
		return false;
...
__forceinline bool isObstructed(const Vec3i& pos, BlockSource* reg, bool allowWater = false){
	...
	AABB aabb{};
	bool hasBox = obs1->getCollisionShape(&aabb, block, reg, &pos);
	...
	for(int x = -1; x <= 1; x++){
		for(int z = -1; z <= 1; z++){
			...
			if(isObstructed(newPos, reg, true)){
				// maybe jump?
				if (isDiagonal) {
					// If any of the upper diagonal blocks are obstructed, the jump isn't safe
					if (isObstructedPlayer(startNode.pos.add(x, 1, 0), reg, true) ||
						isObstructedPlayer(startNode.pos.add(0, 1, z), reg, true))
						continue;

					// If none or both of the lower diagonal blocks are obstructed, the jump is safe
					int diagonalObstructed = 0;
					diagonalObstructed += isObstructed(startNode.pos.add(x, 0, 0), reg, true);
					diagonalObstructed += isObstructed(startNode.pos.add(0, 0, z), reg, true);
					if (diagonalObstructed == 1)
						continue;
				}
			...
			if(!canStandOn(newPos.add(0, -1, 0), reg, isInWater)){
				...
				if(isObstructedPlayer(newPos, reg)){// landing zone
					...
					edges.emplace_back(startNodeRef, findNode(allNodes, jumpPos), PARKOUR_JUMP1_TIME, JoeSegmentType::PARKOUR_JUMP);
					continue;
				}
				...
				if(canJump){
					edges.emplace_back(startNodeRef, findNode(allNodes, newPos), PARKOUR_JUMP1_TIME, JoeSegmentType::PARKOUR_JUMP);
					continue; // we don't need to try a larger jump, we could just walk there
				}
			tryLargerParkourJump:
				// Gives invalid pos warning
				...
				for (int jumpLength = 2; jumpLength <= 3; jumpLength++) {
					for (int i = 0; i <= jumpLength; i++) {
						if (isObstructedPlayer(stuff.at(i), reg) || isObstructed(stuff.at(i).add(0, 2, 0), reg)) {
							goto bad;
						}
					}
					if (canStandOn(stuff.at(jumpLength + 1).sub(0, 1, 0), reg) && !isObstructedPlayer(stuff.at(jumpLength + 1), reg)) {  // Can we actually land here
						edges.emplace_back(startNodeRef, findNode(allNodes, stuff.at(jumpLength + 1)), (jumpLength == 2) ? PARKOUR_JUMP2_TIME : PARKOUR_JUMP3_TIME, JoeSegmentType::PARKOUR_JUMP);
