...
void JoeSegment::init() {
	...
	switch(this->segmentType){
		...
		case JoeSegmentType::JUMP: {
			...
			positions.push_back(mod);
			if (isDiagonal == 2) {
				// movement controller gets thrown the side a little
				mod.x = end.x;
				positions.push_back(mod);
				positions.push_back(mod.sub(0, 1, 0));
				mod.x = start.x;
				mod.z = end.z;
				positions.push_back(mod);
				positions.push_back(mod.sub(0, 1, 0));
		...
		case JoeSegmentType::PARKOUR_JUMP: {
			Vec3 tangentF = end.sub(start).toFloatVector();
			tangentF.y = 0;
			tangentF = tangentF.normalize();
			Vec3i tangent((int)roundf(tangentF.x), 0, (int)roundf(tangentF.z));
			...
			for (int i = 0; i < 5; i++) stuff.push_back(start.add((int)roundf(tangentF.x) * i, 0, (int)roundf(tangentF.z) * i));
			for (int jumpLength = 2; jumpLength <= 3; jumpLength++) {
				positions.push_back(stuff.at(jumpLength + 1).sub(0, 1, 0));
			}
			mod = end;
