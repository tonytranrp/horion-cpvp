...
public:
	...
	virtual __int64 setCustomName(const std::string&); 
...
public:
	Inventory* inventory;  //0x00AC
};

class ScreenReference {
public:
	inline std::string getScreenTypeName() {
		return *reinterpret_cast<std::string*>((uintptr_t)(this) + 0x28);
	}
};

class ControllerPtr {
public:
	float controllerPtrTicks;  // 0x0
private:
	char padding[0x44];

public:
	ScreenReference* ref;  // 0x48
};
...
public:
	...
	ControllerPtr* getControllerPtr() {
		return *reinterpret_cast<class ControllerPtr**>((uintptr_t)(this) + 0x10);
	};

	bool isChestContainer() {
		auto type = this->getControllerPtr()->ref->getScreenTypeName();
		return type.find("chest") != std::string::npos;
	}

	bool isInventory() {
		auto type = this->getControllerPtr()->ref->getScreenTypeName();
		return type.find("crafting.inventory") != std::string::npos;
	}

	std::string getTypeName() {
		return this->getControllerPtr()->ref->getScreenTypeName();
	}
};