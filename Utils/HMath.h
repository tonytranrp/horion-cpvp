...
struct Vec2i {
	...
	Vec2i &operator=(const Vec2i &copy) {
		x = copy.x;
		y = copy.y;
		return *this;
	}

	static Vec2i fromAngle(int angle) {
		return Vec2i((int)-sin(angle), (int)cos(angle));
	}
	...
	Vec2i sub(int ox, int oy) {
		return Vec2i(x - ox, y - oy);
	}
	Vec2i div(int f) {
		return Vec2i(x / f, y / f);
	}

	Vec2i div(const Vec2i &o) {
		return Vec2i(x / o.x, y / o.y);
	}
	Vec2i mul(const Vec2i &o) {
		return Vec2i(x * o.x, y * o.y);
	}
	Vec2i mul(float f) {
		return Vec2i(x * f, y * f);
	}
	Vec2i sub(const Vec2i &o) {
		return Vec2i(x - o.x, y - o.y);
	}
	Vec2i add(const Vec2 &o) {
		return Vec2i(x + o.x, y + o.y);
	}
	Vec2i add(int o) {
		return Vec2i(x + o, y + o);
	}
	Vec2i add(int ox, int oy) {
		return Vec2i(x + ox, y + oy);
	}

	Vec2i normalized() {
		return div((int)magnitude());
	}
	...
	int dot(int ox, int oy) const { return x * ox + y * oy; }

	int dot(const Vec2i &o) const { return x * o.x + y * o.y; }
	...
	float squaredlen() const { return (float)x * (float)x + (float)y * (float)y; }
	float magnitude() const { return sqrtf(squaredlen()); }
};
...
struct Vec3i {
	...
	float squaredlen() const { return x * x + y * y + z * z; }
	float magnitude() const { return sqrtf(squaredlen()); }
	float dist(const Vec3i &e) const {
		return sub(e).magnitude();
	}
};
struct Vec3iHash {
...
struct Vec4 {
	union {
		struct {
			float x, y, z, w; // this should technically be w x y z but no way ima change all dat
		};
		float v[4];
	};
