...
gd_open_gif(const char *fname) {
	...
	errno_t result = _sopen_s(&fd, fname, O_RDONLY, _SH_DENYNO, 0);
	if (fd == -1) return NULL;
#ifdef _WIN32
...
new_table(int key_size) {
	...
	if (table) {
		...
		for (key = 0; key < (1 << key_size); key++)
			table->entries[key] = {1, 0xFFF, (unsigned char)key};
	}
	return table;
}
